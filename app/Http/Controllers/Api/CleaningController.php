...
class CleaningController extends Controller {
    ...
    public function submit(Request $r) {
        ...
        $data = $r->validate(['date'=>'required|date_format:Y-m-d','items'=>'array','labels'=>'array','notes'=>'nullable|string','issues'=>'nullable|string','photos'=>'array']);
        return CleaningLog::updateOrCreate(
            ['location_id'=>$u->location_id,'date'=>$data['date']],
            ['user_id'=>$u->id,'by'=>$u->name,'items'=>$data['items']??[], 'labels'=>$data['labels']??[], 'notes'=>$data['notes']??null,'issues'=>$data['issues']??null,'photos'=>$data['photos']??[]]
        );
    }
